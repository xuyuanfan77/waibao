		...
		if(id == null || id == ''){
			...
			if(play_num > 0 && play_num <= 5000){
				...
				if(issue >= issue_now){
					...
					var selSet1 = $(".J_selSet").eq(0).attr("data-num"),
						selSet2 = $(".J_selSet").eq(1).attr("data-num"),
						selbei = $(".lir-c3 .zn-bei").attr("data-num"),
						autokey = $(".J_autoKey").attr("data-num"),
						selshangxian = $(".lir-c3 .zn-shangxian").val();

					var i = 0;
					$("#my_modes ul:gt(0)").each(function(){
						id_arr[i] = $.trim($(this).find("li:eq(2) span").html());
						win_arr[i]  = $("select[name='drop_win'] option:selected")[i].value;
						lose_arr[i] = $("select[name='drop_lose'] option:selected")[i].value;
						if(autokey==1){
							state_arr[i] = $.trim($(this).find("li:eq(5) a").hasClass("bet-on"));
						}else{
							state_arr[i] = $.trim($(this).find("li:eq(7) a").hasClass("bet-on"));
						}
						i++;
					});

					for(var k= 0;k<id_arr.length;k++){
						var _list = id_arr[k] + "," + win_arr[k] + "," + lose_arr[k] + "," + state_arr[k];
						model_list = model_list + _list + '|';
					}
